...
// Mock the QueryBuilderService
jest.mock("../queryBuilder/service", () => ({
  QueryBuilderService: jest.fn().mockImplementation(() => ({
    buildAgrregationQuery: jest.fn().mockReturnValue([
      {
        $match: {
...
jest.mock("../mongo/models/invoice", () => ({
  Invoices: {
    find: jest.fn(),
    countDocuments: jest.fn(),
  },
...
describe("DashboardService", () => {
  ...
  describe("getCardData", () => {
    it("should return correct card data when all queries succeed", async () => {
      ...
      (Invoices.countDocuments as jest.Mock).mockResolvedValue(20);
      ...
      expect(scans.countDocuments).toHaveBeenCalledWith({
        vendorId: mockVendorId,
      });
      expect(Invoices.countDocuments).toHaveBeenCalledWith({
        vendorId: mockVendorId,
      });
    ...
    it("should handle missing application status counts", async () => {
      ...
      (applications.aggregate as jest.Mock).mockResolvedValue([]);
      (vendorCredits.find as jest.Mock).mockResolvedValue([]);
      (Invoices.countDocuments as jest.Mock).mockResolvedValue(0);
    ...
    it("should propagate errors from database queries", async () => {
      ...
      await expect(
        dashboardService.getCardData({ vendorId: mockVendorId })
